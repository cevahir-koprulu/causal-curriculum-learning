from deep_sprl.environments.unlock.env.minigrid import MiniGridEnv, Ball, Key, Door, COLOR_NAMES, Grid, Box
...
class RoomGrid(MiniGridEnv):
    ...
    This class is only responsible for building the room environment, all basic functions are inherited from MiniGridEnv
    """

    def __init__(self, room_size=7, num_rows=3, num_cols=3, max_steps=100, seed=0, stage="train"):
        assert room_size > 0
        assert room_size >= 3
        assert num_rows > 0
        assert num_cols > 0
    ...
    def _gen_grid(self, width, height, context=None):
        # Create the grid
        ...
        for j in range(0, self.num_rows):
            ...
            for i in range(0, self.num_cols):
                room = Room((i * (self.room_size-1), j * (self.room_size-1)), (self.room_size, self.room_size))
                row.append(room)
        ...
        for j in range(0, self.num_rows):
            ...
            for i in range(0, self.num_cols):
                ...
                # even for a single room, we can set doors
                #room.door_pos[0] = (x_m, self._rand_int(y_l, y_m))
                #room.door_pos[1] = (self._rand_int(x_l, x_m), y_m)
                #room.door_pos[2] = (x_l, self._rand_int(y_l, y_m))
                #room.door_pos[3] = (self._rand_int(x_l, x_m), y_l)

                x_l, y_l = (room.top[0], room.top[1] + 1)
                x_m, y_m = (room.top[0] + room.size[0] - 1, room.top[1] + room.size[1] - 1)
                ...
                if context is None:
                    ...
                    room.door_pos[0] = (x_m, context[0])
                    room.door_pos[1] = (context[1], y_m)
                room.door_pos[2] = (x_l, self._rand_int(y_l, y_m))
                room.door_pos[3] = (self._rand_int(x_l+1, x_m-1), y_l-1)
    ...
    def place_in_room(self, i, j, obj, fixed_row):
        """
        Add an existing object to room (i, j)
        """

        room = self.get_room(i, j)
        pos = self.place_obj(obj, room.top, room.size, reject_fn=reject_next_to, max_tries=1000, fixed_row=fixed_row)
        room.objs.append(obj)
        return obj, pos

    def add_object(self, i, j, kind=None, color=None, fixed_row=None):
        """
        ...
        return self.place_in_room(i, j, obj, fixed_row)
